...
graph TD
    ...
    G -->|Onion Route| H[Entry Node → Middle → Exit]
    H --> I[Polkadot RPC]
```
...
<section style={{
  ...
  <h2>🔗 Transport Layer: Multi-Hop Onion Routing</h2>
  ...
  <p>
    ...
    <code>Client → Entry Node → Middle Node → Exit Node → Polkadot RPC</code>
  </p>
...
<section style={{
  ...
  <h2>🌐 Cross-Chain Mixing Service</h2>
  ...
  <div style={{ marginTop: '1rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
    ...
    >
      {daochainStatus === 'loading' ? 'Submitting XCM...' : '🚀 Submit Cross-Chain Mix Job'}
    </button>
  ...
  {mixJobInfo && (
    <div style={{ marginTop: '1.5rem' }}>
      <h3>✅ MixJob Created on DaoChain:</h3>
      <pre style={{
        padding: '1rem',
...
<section style={{
  ...
  <div style={{
    ...
    <code style={{
      ...
      Other Parachain → XCM Transact → DaoChain.mixJob.submitJob()<br/>
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;↓<br/>
      DaoChain Processes → Mixes Votes → Returns Results via XCM
    </code>
  </div>
</section>
```
...
describe('Cross-Chain Demo', () => {
  it('submits mix job successfully', async () => {
    ...
    const submitButton = screen.getByText('🚀 Submit Cross-Chain Mix Job');
    ...
    expect(screen.getByText('📨 XCM Message Sent:')).toBeInTheDocument();
    expect(screen.getByText('✅ MixJob Created on DaoChain:')).toBeInTheDocument();
  });
});
```
...
test('cross-chain mixing demo works end-to-end', async ({ page }) => {
  ...
  await page.click('text=🚀 Submit Cross-Chain Mix Job');

  // Verify XCM message appears
  await expect(page.locator('text=📨 XCM Message Sent:')).toBeVisible();

  // Verify MixJob response appears
  await expect(page.locator('text=✅ MixJob Created on DaoChain:')).toBeVisible();
